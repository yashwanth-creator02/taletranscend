import { getChapterState, getChapterProgress } 
  from "../../core/services/reader/index.js";
import { getLastReadChapter,getTotalReadTime } from "../../core/services/reader/index.js";
...
export function renderTale(userId,tale, taleId) {
    ...
    document.getElementById("resume-text").innerText = `Resume Fragment ${last}` || "Resume Fragment";
    if (tale.coverUrl) {
...
export function renderChapters(userId , chapters, taleId) {
    ...
    list.innerHTML = chapters.map((ch, idx) =>{ 
        ...
        if (state === "in_progress") icon = `<i data-lucide="circle-dashed" class="w-4 h-4 text-indigo-400"></i>`;
        if (state === "completed") icon = `<i data-lucide="check-circle-2" class="w-4 h-4 text-emerald-500"></i>`;
