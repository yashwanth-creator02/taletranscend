...
export async function publishFullTale() {
  ...
  try {
    ...
    // ── Step 3: Auto-approve ─────────────────────────────────────
    // Moderation gate: update status to 'approved' before community write.
    // Future: check a moderation flag before proceeding.
    ...
    // Uses PATHS.publicTaleChapters() — fixes the previous hardcoded path bug.
    await Promise.all(
      state.chapters.map((ch, idx) =>
        setDoc(doc(db, PATHS.publicTaleChapters(pendingDoc.id) + `/${idx}`), {
          chapterNum: idx,
...
function setPublishButtonsDisabled(disabled) {
  ['publish-btn', 'publish-btn-mobile'].forEach((id) => {
    ...
    // Restore original label — look for the <span> children if present
    const spans = btn.querySelectorAll('span');
    if (spans.length) {
      spans.forEach((s) => {
        if (!s.classList.contains('hidden')) {
          s.textContent = disabled ? 'Publishing…' : (s.dataset.label ?? s.textContent);
        }
      });
    } else {
